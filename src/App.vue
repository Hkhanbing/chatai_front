              ...
              <div class="react-scroll-to-bottom--css-ncqif-79elbk h-full dark:bg-gray-800">
                <div ref="chatContainer" class="react-scroll-to-bottom--css-krija-1n7m0yu">
                  <div  id = "full_div" class="flex flex-col items-center text-sm dark:bg-gray-800">
                      ...
                      <div v-if="conv.role == 'assistant'"
                        ...
                        <div
                          ...
                          <div class="w-[30px] flex flex-col relative items-end">
                            ...
                            <!-- 多个消息 -->
                            <div v-if="conv.content.length > 1"
                              ...
                              <button @click.stop="next(conv)" :disabled="!(conv.idx < conv.content.length - 1)"
                                class="dark:text-white disabled:text-gray-300 dark:disabled:text-gray-400">
                                <svg stroke="currentColor" fill="none" stroke-width="1.5" viewBox="0 0 24 24"
                                  stroke-linecap="round" stroke-linejoin="round" class="h-3 w-3" height="1em" width="1em"
                                  xmlns="http://www.w3.org/2000/svg">
                          ...
                          <div class="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-[calc(100%-115px)]">
                            <div class="flex flex-grow flex-col gap-3">
                              ...
                              <div class="min-h-[20px] flex flex-col items-start gap-4">
                                <div v-html="mdToHtml(conv.content[conv.idx], conv)"
                      ...
                      <div class="md:flex items-start text-center gap-3.5">
                        <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                          ...
                          <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                            <button @click="inputChat('简要解释一下量子计算')"
                              class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md hover:bg-gray-200 dark:hover:bg-gray-900">
                              "简要解释一下量子计算" →
                            </button>
                            <button @click="inputChat('对生日宴会的筹划有什么创意的点子吗')"
                              class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md hover:bg-gray-200 dark:hover:bg-gray-900">
                              "对生日宴会的筹划有什么创意的点子吗？"
                              →
                            </button>
                            <button @click="inputChat('我怎么利用JS发出HTTP请求')"
                              class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md hover:bg-gray-200 dark:hover:bg-gray-900">
                              "我怎么利用JS发出HTTP请求" →
                            </button>
                          </ul>
                        ...
                        <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                          ...
                          <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                            ...
                            <li class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">允许用户选择多款大模型
                            </li>
                            <li class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">重新生成新的对话
                            </li>
                          </ul>
                        </div>
                        <div class="flex flex-col mb-8 md:mb-auto gap-3.5 flex-1">
                          ...
                          <ul class="flex flex-col gap-3.5 w-full sm:max-w-md m-auto">
                            <li class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">也许偶尔会生成不正确的信息
                            </li>
                            <li class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">也许偶尔会产生有害的说明或有偏见的内容
                            </li>
                            <li class="w-full bg-gray-50 dark:bg-white/5 p-3 rounded-md">对2023年后的事件和知识认知有限
                            </li>
                          </ul>
                        </div>
                      </div>
                    ...
                    <div id="need_to_change" class="w-full h-32 md:h-48 flex-shrink-0"></div>
                  </div>
              ...
              <form class="stretch mx-2 flex flex-row gap-3 pt-2 last:mb-2 md:last:mb-6 lg:mx-auto lg:max-w-3xl lg:pt-6">
                <div class="relative flex h-full flex-1 md:flex-col">
                  ...
                  <div class="flex items-center justify-between px-4 py-2">
                    <label for="modelSelection" class="mr-2">模型选择：</label>
                    <select v-model="selectedModel" id="modelSelection" class="rounded-md border-gray-300">
        ...
        <div class="dark hidden bg-gray-900 md:fixed md:inset-y-0 md:flex md:w-[260px] md:flex-col">
          <div class="flex h-full min-h-0 flex-col ">
            <div ref="menu" class="scrollbar-trigger flex h-full w-full flex-1 items-start border-white/20">
              ...
              <nav ref="navEle" class="flex h-full flex-1 flex-col space-y-1 p-2" style="background-color: #393e46;">
              ...
              <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div v-if="popupShow"
                  ...
                  <div class="prose dark:prose-invert">
                    <div class="mb-5">
                      <h2 class="!mt-4 font-normal !mb-2"><b>一站式大模型访问</b></h2>
                    </div>
                    ...
                    <div class="flex gap-4 flex-col text-sm">
                      <div class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                        ...
                        <div class="flex-1 leading-5">
                          欢迎来到一站式大模型访问，点击左上角即可创建新对话，右下角可选择对应大模型。
                        </div>
                      </div>
                      <div class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                        <div class="w-10 text-2xl text-center">🔬</div>
                        <div class="flex-1 leading-5">目前已开放大模型：文心一言、智谱清言、通义千问
                        </div>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    stopChat() {
      var that = this;
      this.axios.post(`/api/stop/chat/${this.cid}`, {})
        .then((result) => {
          var rconv = that.conversation[that.conversation.length - 1];
          rconv["loading"] = false;
          that.convLoading = false;

          if (that.conversation.length == 2 && rconv["content"].length == 1) {
            var newConv = {
              "id": that.cid,
              "title": "New chat"
            }
    ...
    changeTheme(theme) {
      ...
      //以下几句是新加的，为了适配夜间、日间模式的选择框
      var full_div = document.getElementById('full_div');
      var selectBox = document.getElementById('modelSelection');
    ...
    initConvs(convs) {
      ...
      for (let i = 0; i < convs.length; i++) {
        ...
        conv["idx"] = conv["content"].length - 1;
      }
    ...
    suitable(idx, conv, suit) {
      ...
      this.axios.post(`/api/assistant/suitable/${this.cid}`, cdate)
        .then((result) => {
          console.log(result);
    ...
    next(conv) {
      if (conv.idx == conv["content"].length - 1) {
        return;
      }
      conv.idx++;
      this.refrechConversation();
    ...
    chatRepeat() {
      ...
      // 删除conversation数组的末尾元素
      if (this.conversation.length > 1) {
        this.conversation.pop();
        this.chatMsg = this.conversation[this.conversation.length - 1].content;
        console.log("ChatMsg: ");
        console.log(JSON.stringify(this.chatMsg));
        this.conversation.pop();
      }
    ...
    send() {
      ...
      this.convLoading = true;
      let chatMsg = this.chatMsg.trim().replace(/\n/g, "");
      this.chatMsg = "";

      console.log("new chatMsg: ");
      console.log(chatMsg);
      ...
      fetch(`http://localhost:8081/message`, {
        ...
        .then(data => {
          ...
          const conv = that.conversation[that.conversation.length - 1];
          if (data.msg === "success") {
            ...
            console.log(data);
            var aiResponse = data.data.result;
            conv.content[0] = aiResponse; // response
            console.log("Now into refresh");
            that.refrechConversation();
            that.source = undefined;
            console.log(JSON.stringify(that.conversation));
            return;
          }
    ...
    generateConvTitle(conv) {
      ...
      // 使用fetch发送POST请求
      fetch(`http://localhost:8081/api/chat/title/${this.cid}`, {
        method: 'POST', // 指定请求方法为POST
        headers: {
          'Content-Type': 'application/json', // 设置内容类型为JSON
        },
        body: JSON.stringify({ conversationId: this.cid }), // 将会话ID作为请求体发送
      })
        .then(response => response.json()) // 解析JSON响应
        .then(data => {
          // 假设后端以某种形式发送完整的标题或数据块
          if (data.status === 'success') {
            conv.title = data.title; // 假设后端发送的数据中包含title
    ...
    loadId() {
      var that = this;
      this.axios.post(`http://localhost:8081/sessions`, {})
        .then((result) => {
          console.log(result);
    ...
    selectConversation(conv, loadConv) {
      ...
      this.axios.get(`http://localhost:8081/message?sessionId=${conv.id}`)
        .then((result) => {
          console.log(result);
          var resp = result.data;
          ...
          for (let i = 1; i < content.length; i += 2) {
            content[i] = { "idx": 0, "loading": false, "role": content[i].role, "suitable": [0], "content": [content[i].content] };
